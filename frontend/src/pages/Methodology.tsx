              ...
              <div className="bg-ink-900 p-6">
                ...
                <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
                  {Object.entries(data.definitions).map(([k, v]) => (
                    <div key={k} className="rounded-lg bg-ink-800/60 p-3">
                      <div className="font-mono text-sm font-semibold text-ink-100">{k}</div>
                      <div className="mt-0.5 text-xs text-ink-300">{v}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {data.steps.map((step) => (
                <div key={step.step} className="card relative p-4">
                  ...
                  {step.step < data.steps.length && (
                    <ArrowRight className="absolute -right-2 top-1/2 hidden h-4 w-4 -translate-y-1/2 text-ink-300 lg:block" />
                  )}
                </div>
              ))}
            </div>
